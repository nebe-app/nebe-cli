...
module.exports = class Install {
	async handle() {
		...
		const homeDir = path.join(os.homedir(), `nebe-visuals${suffix}`);
		const projectsDir = path.join(os.homedir(), `Projects`, `nebe-visuals${suffix}`);
		const cwdDir = process.cwd();
		const cwdNestDir = path.join(process.cwd(), `nebe-visuals${suffix}`);

		const choices = [
				{ value: 'homeDir', name: `${ homeDir } (~/nebe-visuals${suffix})` },
				{ value: 'projectsDir', name: `${ projectsDir } (~/Projects/nebe-visuals${suffix})` },
				{ value: 'cwdDir', name: `${ cwdDir } (Current folder)` },
				{ value: 'cwdNestDir', name: `${ cwdNestDir } (Create folder /nebe-visuals${suffix} in current folder)` }
			];
		...
		switch (rootAnswer.root) {
			...
			case 'cwdNestDir':
				try {
					await fs.promises.mkdir(path.join('.', `nebe-visuals${suffix}`));
				} catch (error) {
				}
				dir = cwdNestDir;
				break;
			case 'homeDir':
				try {
					await fs.promises.mkdir(path.join(os.homedir(), `nebe-visuals${suffix}`));
				} catch (error) {
				}
				dir = homeDir;
				break;
			case 'projectsDir':
				...
				try {
					await fs.promises.mkdir(path.join(os.homedir(), 'Projects', `nebe-visuals${suffix}`));
				} catch (error) {
				}
				dir = projectsDir;
				break;
