...
module.exports = class Dev {
	...
	async handle() {
		...
		reset.createPackageJson()
		reset.createHtmlNanoRc();
			...
			try {
				...
				bundler.on('bundled', (bundle) => {
					...
					const visualClientScript = this.local
						...
						: `<script src="https://assets.imagelance.com/helpers/serving/dist/visual-client.min.js?cb=${new Date().getTime()}"></script>`;
					...
					const visualHelperUrl = this.local
						? 'http://localhost:1235/'
						: 'https://assets.imagelance.com/helpers/utils/dist/';
