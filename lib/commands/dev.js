...
module.exports = class Dev {
	...
	async handle() {
		...
		const homeFolder = path.join(os.homedir(), '.nebe');

		process.chdir(homeFolder);
		...
		if (lastDev) {
			...
			const lastVisualContent = root + '/' + lastDev;
		 ...
		 * Configs
		 */
		const configs = glob.sync(`${root}/${visualPath}/config.*`);

		for (let i = 0; i < configs.length; i++) {
			const path = configs[i];
			const relativePath = path.toString().replace(`${root}/${visualPath}/`, '');
			fs.copySync(path, `${root}/dist/${relativePath}`);
		}
		 ...
		 * Include folders
		 */
		const includes = glob.sync(`${root}/${visualPath}/include/`);
		...
		const schemaPath = `${root}/${visualPath}/schema.json`;
		...
		for (let i = 0; i < folders.length; i++) {
			...
			const folder = entryPoint.toString()
				.replace(`${root}/${visualPath}/`, '')
				.replace('/index.html', '');

			try {
				const options = {
					outDir: `${root}/dist/${folder}`,
					outFile: 'index.html',
					publicUrl: '/',
					watch: true,
					cache: true,
				...
				bundler.on('bundled', (bundle) => {
					let markupContents = fs.readFileSync(`${root}/dist/${folder}/index.html`).toString();
					markupContents = markupContents
						.replace(/<!--NEBE_DEMO_FILL-->.+<!--\/NEBE_DEMO_FILL-->/gms, '')
						.replace(`</body>`, `\n<!--NEBE_DEMO_FILL-->\n${fill}\n<!--/NEBE_DEMO_FILL-->\n</body>`);
					fs.writeFileSync(`${root}/dist/${folder}/index.html`, markupContents);
