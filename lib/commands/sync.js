...
module.exports = class Sync {
	async handle() {
			...
			for (let i in reversedVisualKeys) {
				...
				if (files.length > 0) {
					...
					try {
						...
						await git.clone(visualData.origin, repoPath, {'--depth': '1'});
						if (isVerbose) {
				...
				try {
					...
					if (status.current !== 'master' && status.current !== 'master.test') {
						if (canSwitchBranch) {
							...
							if (isVerbose) {
								console.log(chalk.yellow(repoPath + ' is not on master, is on ' + status.current));
							}
							continue;
						}
					}
		...
		const url = isSazka() ? 'https://sazka.nebe.app/developer/sync' : 'https://client.nebe.app/developer/sync';
